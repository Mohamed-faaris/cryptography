...
```
This script continuously opens system applications in a loop.
```
...
```bash
ip a
sudo snort -i <interface_name> -A console -c /etc/snort/snort.conf
...
```
Use a different terminal and run: ping 8.8.8.8
```
...
```
RKHunter is used to check for rootkits and suspicious system changes.
```
...
| 9 | Trojan | Malware Demo | Batch Loop |
| 10 | Snort | IDS | Rules |
| 11 | RKHunter | Host IDS | Rootkit Scan |
